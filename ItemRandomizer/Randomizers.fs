...
module NewRandomizer =
    ...
    let hardCode (itemType:ItemType) (locationName:string) (items:Item list byref) (itemLocations:ItemLocation list byref) (itemPool:Item list byref) (locationPool:Location list) =
      let item = List.find (fun i -> i.Type = itemType) Items.Items
      let cl = List.find(fun (location:Location) -> locationName = location.Name) locationPool
      let itemLocation = placeSpecificItemAtLocation item cl
